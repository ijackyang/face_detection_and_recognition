...
def centers_gather(centers,labels):
    labels = tf.cast(labels,tf.int32)
    batch_centers = tf.gather(centers,labels)
    return batch_centers

def get_centers(shape=[10575,256]):
    return tf.Variable(tf.zeros(shape),name='centers',dtype=tf.float32,trainable=False)
...
def resize_bbox(bbox,scale,padding):
    bbox = bbox*scale
    bbox = bbox.astype(np.uint)
    bbox = np.array([np.array([ibbox[0]+padding[0][0],ibbox[1]+padding[1][0],ibbox[2]+padding[0][0],ibbox[3]+padding[1][0]]) for ibbox in bbox])
    return bbox
